...
class TQQQBacktest2025Original:
    """TQQQ 2025年度回測引擎（原始策略參數版）"""
    ...
    def check_entry_signal(self, df: pd.DataFrame, idx: int) -> tuple:
        ...
        if zscore < -self.config['entry_zscore']:
            return 'long', f"Z-Score({zscore:.2f}) < -{self.config['entry_zscore']}"
        
        # 做空條件: Z-Score > 2.0
        if zscore > self.config['entry_zscore']:
            return 'short', f"Z-Score({zscore:.2f}) > {self.config['entry_zscore']}"
    ...
    def run_backtest(self, df: pd.DataFrame, start_date: str, end_date: str) -> dict:
        ...
        print(f"原始策略參數: entry_zscore={self.config['entry_zscore']}, exit_zscore={self.config['exit_zscore']}")
        print(f"倉位比例: {self.config['position_pct']*100}%, 時間止損: {self.config['time_stop_days']}天")
...
def main():
    ...
    # 創建回測引擎
    backtest = TQQQBacktest2025Original(**config)
    ...
    output_file = f"{output_dir}/backtest_2025_original.json"

...
class TQQQBacktestStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        print(f"TQQQ策略初始化: 倉位{self.config.position_pct*100}%, Z-Score±{self.config.entry_zscore}, VIX閾值{self.config.vix_threshold}")
    ...
    def generate_signal(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        # ========== 做多條件檢查（優化版）==========
        # 條件：Z-Score超賣 + RSI超賣 + 成交量確認 + 市場狀態允許
        if zscore < -self.config.entry_zscore:
            # 放寬RSI條件：從 <35 改為 <40
            rsi_condition = rsi < 40  # 放寬條件
            volume_condition = volume > volume_ma * 0.7  # 放寬成交量條件
            market_condition = market_state in ['bull', 'sideways']  # 允許bull和sideways
            
            if rsi_condition and volume_condition and market_condition:
                return {
        ...
        # ========== 做空條件檢查（優化版）==========
        # 條件：Z-Score超買 + RSI超買 + 成交量確認 + 市場狀態允許
        if zscore > self.config.entry_zscore:
            # 放寬RSI條件：從 >65 改為 >60
            rsi_condition = rsi > 60  # 放寬條件
            volume_condition = volume > volume_ma * 0.7  # 放寬成交量條件
            # 放寬市場條件：允許所有市場狀態（因為2023年主要是bull市場）
            market_condition = True  # 允許所有市場狀態做空
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        """獲取策略信息"""
        return {
            'name': 'TQQQ Long/Short Strategy (框架A優化版)',
            ...
            'notes': '優化版：放寬RSI條件(40/60)，放寬市場狀態條件'
        }
...
def run_backtest_2023():
    ...
    print("="*70)
    print("TQQQ Long/Short Strategy - 2023年度回測（優化版）")
    print("="*70)
    print("回測期間: 2023-03-27 至 2024-03-27")
    print("策略版本: 框架A優化版（放寬條件）")
    print("="*70)
    ...
    config = {
        ...
        'lookback_period': 60,
        'rsi_period': 14,
        'rsi_overbought': 60.0,  # 放寬從65
        'rsi_oversold': 40.0,    # 放寬從35
        'take_profit_pct': 0.05,
        'stop_loss_pct': 0.03,
        'time_stop_days': 3,
        'vix_threshold': 25.0,
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果
    json_file = f'{report_dir}/backtest_2023_{timestamp}.json'
    result_dict = {
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/equity_curve_2023_{timestamp}.csv'
        result.equity_curve.to_csv(equity_file)
        print(f"📈 權益曲線已保存: {equity_file}")


if __name__ == '__main__':
    result = run_backtest_2023()

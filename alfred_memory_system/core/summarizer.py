"""
Summarizer - 摘要生成器
從 memory-system-plugin 整合
負責自動生成對話摘要，提取決策、行動項目、思考過程、情感脈絡

注意：此為可選功能，默認關閉。不依賴 ChromaDB，使用現有 SQLite 存儲。
"""
...
class Summarizer:
    ...
    def __init__(self, storage_path: Optional[Path] = None, 
        ...
        Args:
            storage_path: 存儲路徑，默認為 ~/openclaw_workspace/memory/summaries
        ...
        if storage_path is None:
            storage_path = Path.home() / "openclaw_workspace" / "memory" / "summaries"
    ...
    def generate_summary(
        ...
        Returns:
            ConversationSummary 對象，如果未啟用則返回 None
        """
        if not self.enabled:
            return None
    ...
    def save_summary(self, summary: ConversationSummary) -> Optional[Path]:
        """
        保存摘要到文件
        
        Returns:
            保存的文件路徑，如果未啟用則返回 None
        """
        if not self.enabled:
            return None
...
def create_summarizer(storage_path: Optional[str] = None, 
                      enabled: bool = False) -> Summarizer:
    """創建摘要生成器實例"""
    path = Path(storage_path) if storage_path else None
    return Summarizer(path, enabled=enabled)


def quick_summarize(conversation_text: str, session_id: str = "default",
                    title: Optional[str] = None, enabled: bool = False) -> Optional[ConversationSummary]:
    """快速生成摘要"""
    summarizer = create_summarizer(enabled=enabled)
...
def summarize_and_save(conversation_text: str, session_id: str,
                       title: Optional[str] = None, 
                       enabled: bool = False) -> Tuple[Optional[ConversationSummary], Optional[Path]]:
    """生成摘要並保存"""
    summarizer = create_summarizer(enabled=enabled)

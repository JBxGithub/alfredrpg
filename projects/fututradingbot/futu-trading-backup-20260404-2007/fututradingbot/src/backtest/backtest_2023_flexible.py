...
class FlexibleArbitrageStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 市場狀態判斷參數
        self.ma_period = 60          # 60日均線（適應較短數據）
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 計算60日均線
        df['ma60'] = df['close'].rolling(window=self.ma_period).mean()
        
        return df
    
    def determine_market_state(self, price: float, ma60: float, vix: Optional[float] = None) -> MarketState:
        """
        判斷市場狀態
        ...
        if vix is None:
            ...
            if deviation > 0.03:  # 價格高於60日均線3%以上 - 牛市但允許雙向
                return MarketState(
                    state="bull",
                    primary_direction="both",  # 修改：牛市也允許雙向交易
                    zscore_threshold=self.zscore_threshold_bull,
                    position_pct=0.70
                )
            elif deviation < -0.03:  # 價格低於60日均線3%以上 - 熊市但允許雙向
                return MarketState(
                    state="bear",
                    primary_direction="both",  # 修改：熊市也允許雙向交易
        ...
        # 有VIX數據時的判斷
        # 牛市條件: 價格在60日均線之上 + VIX低
        if price > ma60 and vix < self.vix_bull_threshold:
            return MarketState(
                state="bull",
                primary_direction="both",  # 修改：牛市也允許雙向交易
                zscore_threshold=self.zscore_threshold_bull,
        ...
        # 熊市條件: 價格在60日均線之下 + VIX高
        elif price < ma60 and vix > self.vix_bear_threshold:
            return MarketState(
                state="bear",
                primary_direction="both",  # 修改：熊市也允許雙向交易
                zscore_threshold=self.zscore_threshold_bear,
...
def save_results(result: BacktestResult, strategy: FlexibleArbitrageStrategy):
    ...
    # 保存JSON結果
    json_file = f'{report_dir}/backtest_2023_flexible.json'
    result_dict = {

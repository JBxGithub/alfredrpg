"""
TQQQ策略2025年度回測（靈活套利策略）
回測期間: 2025-01-01 至 2025-12-31 (全年)
策略: 靈活套利策略 - 根據市場狀態動態調整方向

## 策略邏輯
- 牛市 (價格 > 200日均線 + 5%): 只做多，Z-Score閾值 1.2
- 熊市 (價格 < 200日均線 - 5%): 只做空，Z-Score閾值 1.2
- 震盪市 (價格在200日均線 ±5%內): 雙向交易，Z-Score閾值 1.5
...
class FlexibleArbitrageStrategy:
    ...
    def generate_report(self, df: pd.DataFrame) -> dict:
        ...
        report = {
            ...
            'strategy_name': '靈活套利策略 (Flexible Arbitrage)',
...
def main():
    ...
    # 創建回測引擎
    backtest = FlexibleArbitrageStrategy(**config)
    ...
    output_file = f"{output_dir}/backtest_2025_flexible.json"

...
class SixLoopBacktestV5:
    ...
    def __init__(self, start_date='2019-01-01', end_date='2024-12-31', initial_capital=100000):
        ...
        # 核心參數
        self.qqq_stop_loss = 0.02           # 日間止損: QQQ -2%
        self.qqq_trailing_stop = 0.02       # 高位回落止損: 從高位回落 2%
        self.take_profit_pct = 0.10         # 止盈: TQQQ +10%
        self.reeval_days = 7                # 7天重新評估
        self.max_position_pct = 0.95
        
        # 交易成本
        self.commission_rate = 0.001        # 0.1% 手續費
    ...
    def run_backtest(self):
        """運行回測"""
        print("\n" + "="*60)
        print("六循環系統回測 - V5 (最終完整版)")
        print("="*60)
        print(f"日間止損: QQQ 入場價 -2%")
        print(f"高位回落止損: 從持倉高位回落 -2%")
        print(f"止盈: TQQQ +10%")
        ...
        for i in range(len(common_dates)):
            ...
            if self.positions > 0:
                ...
                # ===== 第二優先：高位回落止損 (從高位回落 2%) =====
                    ...
                    stop_reason = f'從高位{self.highest_qqq_price:.2f}回落{abs(qqq_change_from_high)*100:.1f}%'
                
                # ===== 第三優先：止盈 (TQQQ +10%) =====
                elif tqqq_change >= self.take_profit_pct:
                    signal = 'SELL_PROFIT'
                    stop_reason = f'TQQQ盈利{tqqq_change*100:.1f}%'
                
                # ===== 第四：7天重新評估 =====
                elif self.holding_days >= self.reeval_days:
                    if final_score > 60:
                        signal = 'HOLD_BULL_CONTINUE'
                        self.holding_days = 0  # 重置，再計7天
                    else:
            ...
            if signal == 'BUY' and self.positions == 0:
                ...
                if shares > 0:
                    ...
                    self.highest_qqq_price = qqq_price  # 初始化最高價
            ...
            daily_results.append({
                ...
                'highest_qqq': self.highest_qqq_price if self.positions > 0 else 0
            })
    ...
    def calculate_metrics(self):
        ...
        print(f"\n【賣出類型】")
        print(f"日間止損 (QQQ-2%): {len(qqq_stops)} 次")
        print(f"高位回落止損 (-2%): {len(trailing_stops)} 次")
        ...
        print(f"交易成本影響: -${self.total_commission:,.2f} ({self.total_commission/self.initial_capital*100:.2f}%)")
    ...
    def save_results(self):
        ...
        self.daily_results.to_csv(f'backtest/results/v5_daily_{timestamp}.csv', index=False)
        
        if self.trades:
            trades_df = pd.DataFrame(self.trades)
            trades_df.to_csv(f'backtest/results/v5_trades_{timestamp}.csv', index=False)
        
        with open(f'backtest/results/v5_metrics_{timestamp}.json', 'w') as f:
            json.dump(self.metrics, f, indent=2, default=str)
        
        print(f"\n結果已保存: backtest/results/v5_*_{timestamp}.*")

def main():
    backtest = SixLoopBacktestV5(

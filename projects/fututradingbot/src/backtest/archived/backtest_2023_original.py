...
class TQQQBacktestStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        print(f"TQQQ策略初始化: 倉位{self.config.position_pct*100}%, Z-Score±{self.config.entry_zscore}, 時間止損{self.config.time_stop_days}天")
    ...
    def check_vix_condition(self, data: pd.DataFrame) -> bool:
        """檢查VIX條件 - 波動率過高時不交易"""
        if len(data) < 20:
            return True
        
        current_volatility = data['volatility'].iloc[-1] if 'volatility' in data.columns else 0
        return current_volatility < self.config.vix_threshold
    ...
    def generate_signal(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        vix_ok = self.check_vix_condition(df)
                ...
                if volume > volume_ma * 0.8:
                    if market_state in ['bull', 'sideways']:
                        return {
                            ...
                            'reason': f"Z-Score({zscore:.2f})超賣+RSI({rsi:.1f})低+{market_state}市+Vol{volatility:.1f}"
                        }
                ...
                if volume > volume_ma * 0.8:
                    if market_state in ['bear', 'sideways']:
                        return {
                            ...
                            'price': current['close'],
                            'confidence': min(abs(zscore) / 3, 0.95),
                            'reason': f"Z-Score({zscore:.2f})超買+RSI({rsi:.1f})高+{market_state}市+Vol{volatility:.1f}"
                        }
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        """獲取策略信息"""
        return {
            'name': 'TQQQ Long/Short Strategy (原始參數)',
            ...
            'require_market_state': self.config.require_market_state
        }
...
def run_backtest_2023_original():
    """執行2023年度回測 - 原始策略參數"""
    
    print("="*70)
    print("TQQQ Long/Short Strategy - 2023年度回測 (原始參數)")
    print("="*70)
    print("回測期間: 2023-01-01 至 2023-12-31")
    print("策略參數: Entry Z-Score 2.0 | Exit Z-Score 0.5 | Position 50% | Time Stop 3天")
    print("="*70)
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果 - 使用指定文件名
    json_file = f'{report_dir}/backtest_2023_original.json'
    result_dict = {
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/equity_curve_2023_original.csv'
        result.equity_curve.to_csv(equity_file)
        print(f"📈 權益曲線已保存: {equity_file}")
    
    # 保存文本報告
    report_file = f'{report_dir}/backtest_2023_original_report.txt'
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("TQQQ Long/Short Strategy - 2023年度回測報告 (原始參數)\n")
        f.write("="*70 + "\n\n")
        f.write(f"回測期間: {result.start_date.strftime('%Y-%m-%d')} ~ {result.end_date.strftime('%Y-%m-%d')}\n")
        f.write(f"策略參數:\n")
        f.write(f"  - Entry Z-Score: 2.0\n")
        f.write(f"  - Exit Z-Score: 0.5\n")
        f.write(f"  - Position: 50%\n")
        f.write(f"  - Time Stop: 3天\n\n")
        f.write("-"*70 + "\n")
        f.write("績效摘要:\n")
        f.write("-"*70 + "\n")
        ...
        if result.trades:
            ...
            for i, t in enumerate(result.trades, 1):
                ...
                f.write(f"{i}. {emoji} [{t.direction.upper()}] {t.entry_date.strftime('%Y-%m-%d')} -> {t.exit_date.strftime('%Y-%m-%d')}\n")
                f.write(f"   進場: ${t.entry_price:.2f} | 出場: ${t.exit_price:.2f} | 股數: {t.shares}\n")
                f.write(f"   P&L: ${t.pnl:,.2f} ({t.pnl_pct*100:+.2f}%) | 原因: {t.exit_reason}\n\n")
    
    print(f"📝 文本報告已保存: {report_file}")


if __name__ == '__main__':
    result = run_backtest_2023_original()

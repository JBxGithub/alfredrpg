...
class SixLoopBacktestV4:
    ...
    def __init__(self, start_date='2019-01-01', end_date='2024-12-31', initial_capital=100000):
        ...
        # 核心參數
        self.qqq_stop_loss = 0.02      # QQQ 跌 2% 強制止損
        self.take_profit_pct = 0.10    # TQQQ 升 10% 止盈
    ...
    def generate_signal(self, abs_score, ref_score, current_position):
        ...
        if current_position == 0:
            ...
            # 有持倉時，7天重新評估
            return 'EVALUATE', final_score
    
    def run_backtest(self):
        """運行回測"""
        print("\n" + "="*60)
        print("六循環系統回測 - V4 (靚仔最終版)")
        print("="*60)
        print(f"止損: QQQ -2% (強制)")
        print(f"止盈: TQQQ +10%")
        print(f"7天評估: 牛市續持(>60)，熊市觀望(<60)")
        ...
        for i in range(len(common_dates)):
            ...
            if self.positions > 0:
                ...
                # 2. 計算 TQQQ 變化
                tqqq_change_pct = (tqqq_price - self.entry_price) / self.entry_price
                ...
                # ===== 第二優先：止盈 (TQQQ +10%) =====
                elif tqqq_change_pct >= self.take_profit_pct:
                    signal = 'SELL_PROFIT'
    ...
    def save_results(self):
        ...
        self.daily_results.to_csv(f'backtest/results/v4_daily_{timestamp}.csv', index=False)
        
        if self.trades:
            trades_df = pd.DataFrame(self.trades)
            trades_df.to_csv(f'backtest/results/v4_trades_{timestamp}.csv', index=False)
        
        with open(f'backtest/results/v4_metrics_{timestamp}.json', 'w') as f:
            json.dump(self.metrics, f, indent=2, default=str)
        
        print(f"\n結果已保存: backtest/results/v4_*_{timestamp}.*")

def main():
    backtest = SixLoopBacktestV4(

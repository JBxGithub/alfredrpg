...
class TrendStrategy(BaseStrategy):
    ...
    def _check_entry_conditions(
        ...
        signal = TradeSignal(
            ...
            metadata={
                ...
                'stop_loss': self._calculate_stop_loss(df, current_price, 'long'),
                'take_profit': self._calculate_take_profit(df, current_price, 'long')
            }
        )
    ...
    def _calculate_stop_loss(
        ...
        if self.config['use_atr_stop']:
            ...
            if position_type == 'long':
                return entry_price - atr_value * self.config['atr_multiplier_sl']
            else:
                return entry_price + atr_value * self.config['atr_multiplier_sl']
        else:
            # 使用固定比例止損
    ...
    def _calculate_take_profit(
        ...
        if self.config['use_atr_stop']:
            ...
            if position_type == 'long':
                return entry_price + atr_value * self.config['atr_multiplier_tp']
            else:
                return entry_price - atr_value * self.config['atr_multiplier_tp']
        else:
            # 使用固定比例止盈

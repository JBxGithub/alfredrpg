...
class TQQQStrategyConfig:
    """TQQQ策略配置 - 放寬版"""
    entry_zscore: float = 1.5          # 放寬Z-Score閾值
    ...
    rsi_period: int = 14
    rsi_overbought: float = 65.0       # 放寬RSI至65
    rsi_oversold: float = 35.0         # 放寬RSI至35
...
class TQQQBacktestStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        print(f"TQQQ策略初始化(放寬版): 倉位{self.config.position_pct*100}%, Z-Score±{self.config.entry_zscore}, RSI超賣<{self.config.rsi_oversold}/超買>{self.config.rsi_overbought}")
...
def run_backtest_2023_relaxed():
    """執行2023年度回測（放寬版）"""
    
    print("="*70)
    print("TQQQ Long/Short Strategy - 2023年度回測（放寬版）")
    print("="*70)
    ...
    # 策略配置（放寬版）
    config = {
        'entry_zscore': 1.5,           # 放寬Z-Score
        'exit_zscore': 0.5,
        'stop_loss_zscore': 3.5,
        'position_pct': 0.50,
        'lookback_period': 60,
        'rsi_period': 14,
        'rsi_overbought': 65.0,        # 放寬RSI
        'rsi_oversold': 35.0,          # 放寬RSI
        'take_profit_pct': 0.05,
        'stop_loss_pct': 0.03,
        'time_stop_days': 3,
        'vix_threshold': 25.0,
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果
    json_file = f'{report_dir}/backtest_2023_relaxed.json'
    result_dict = {
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/equity_curve_2023_relaxed.csv'
        result.equity_curve.to_csv(equity_file)
        print(f"📈 權益曲線已保存: {equity_file}")


if __name__ == '__main__':
    result = run_backtest_2023_relaxed()

...
class SixLoopBacktestV7:
    ...
    def __init__(self, start_date='2019-01-01', end_date='2024-12-31', initial_capital=100000):
        ...
        # V7: 放寬止損到 3%
        self.qqq_stop_loss = 0.03           # 日間止損: QQQ -3% (放寬)
        self.qqq_trailing_stop = 0.03       # 高位回落止損: -3% (放寬)
        self.take_profit_pct = 0.10         # 止盈: TQQQ +10% (保持)
    ...
    def run_backtest(self):
        """運行回測"""
        print("\n" + "="*60)
        print("六循環系統回測 - V7 (放寬止損版)")
        print("="*60)
        print(f"日間止損: QQQ 入場價 -3% (放寬)")
        print(f"高位回落止損: 從持倉高位回落 -3% (放寬)")
        print(f"止盈: TQQQ +10%")
    ...
    def save_results(self):
        ...
        self.daily_results.to_csv(f'backtest/results/v7_daily_{timestamp}.csv', index=False)
        
        if self.trades:
            trades_df = pd.DataFrame(self.trades)
            trades_df.to_csv(f'backtest/results/v7_trades_{timestamp}.csv', index=False)
        
        with open(f'backtest/results/v7_metrics_{timestamp}.json', 'w') as f:
            json.dump(self.metrics, f, indent=2, default=str)
        
        print(f"\n結果已保存: backtest/results/v7_*_{timestamp}.*")

def main():
    backtest = SixLoopBacktestV7(

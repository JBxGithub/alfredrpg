"""
TQQQ 2022年度回測腳本 - 原始策略參數
回測期間: 2022-01-01 至 2022-12-31
策略參數: Entry Z-Score 2.0, Exit Z-Score 0.5, Position 50%, Time Stop 3天
"""
...
class TQQQOriginalBacktestStrategy:
    """TQQQ回測專用策略（原始參數版）"""
    ...
    def __init__(self):
        ...
        print(f"TQQQ策略初始化 (原始參數): 倉位{self.position_pct*100}%, Entry Z-Score±{self.entry_zscore}, Exit Z-Score±{self.exit_zscore}, Time Stop {self.time_stop_days}天")
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        """獲取策略信息"""
        return {
            'name': 'TQQQ Long/Short Strategy (原始參數版)',
            'version': '1.0.0',
...
class TQQQBacktestEngine:
    ...
    def __init__(
        self,
        strategy: TQQQOriginalBacktestStrategy,
...
def save_results(result: BacktestResult, strategy: TQQQOriginalBacktestStrategy):
    """保存回測結果"""
    ...
    # 保存JSON結果（指定文件名）
    json_file = f'{report_dir}/backtest_2022_original.json'
    result_dict = {
    ...
    report_file = f'{report_dir}/backtest_2022_original_report_{timestamp}.txt'
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("TQQQ Long/Short Strategy - 2022年度回測報告 (原始參數)\n")
        f.write("="*70 + "\n\n")
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/backtest_2022_original_equity_{timestamp}.csv'
...
def run_backtest():
    ...
    print("="*70)
    print("TQQQ Long/Short Strategy - 2022年度回測 (原始參數)")
    print("="*70)
    ...
    # 執行回測（2022年度: 2022-01-01 至 2022-12-31）
    result = engine.run(
        data,

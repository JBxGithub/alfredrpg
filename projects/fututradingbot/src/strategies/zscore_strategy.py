...
class ZScoreStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        super().__init__(name="ZScore_MeanReversion", config=default_config)
        
        self.period = default_config['period']
        self.upper_threshold = default_config['upper_threshold']
        self.lower_threshold = default_config['lower_threshold']
        self.exit_threshold = default_config['exit_threshold']
        self.position_pct = default_config['position_pct']
        self.use_mtf = default_config['use_mtf']
        self.mtf_min_score = default_config['mtf_min_score']
        
        # 初始化MTF分析器
        if self.use_mtf:
            self.mtf_analyzer = MTFAnalyzer()
            logger.info("ZScore策略: MTF分析已啟用")
        else:
            self.mtf_analyzer = None
        
        self.name = "ZScore_MeanReversion_MTF"
        self.version = "2.0.0"
    ...
    def on_data(self, data: Dict[str, Any]) -> Optional[TradeSignal]:
        ...
        # 生成信號
        signal_dict = self.generate_signal(df)
        ...
        # 轉換為TradeSignal
        trade_signal_type = SignalType.BUY if signal_type == 1 else SignalType.SELL
        
        return TradeSignal(
            signal=trade_signal_type,
            symbol=symbol,
            price=signal_dict.get('price', 0),
            qty=100,  # 默認數量
            confidence=signal_dict.get('strength', 0.5),
            reason=f"ZScore策略 | {signal_dict.get('signal_text')} | Z:{signal_dict.get('zscore')}",

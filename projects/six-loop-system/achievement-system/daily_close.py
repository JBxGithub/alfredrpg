...
def save_daily_record(target_date: date, daily_pnl: float, win_rate: float,
                     max_drawdown: float, badges: List[Dict]) -> bool:
    conn = get_db_connection()
    cur = conn.cursor()
...
def run_daily_close(target_date: Optional[date] = None) -> Dict:
    ...
    save_daily_record(target_date, daily_pnl, win_rate, max_drawdown, badges)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Daily Close Tasks")

"""
TQQQ 2021年度回測腳本 - TQQQ 2021 Year Backtest
執行TQQQ單邊多空策略的2021年度回測（2021-03-27 至 2022-03-27）
...
class TQQQBacktestStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        print(f"📋 策略配置:")
        print(f"   Entry Z-Score: ±{self.config.entry_zscore}")
        print(f"   Exit Z-Score: ±{self.config.exit_zscore}")
        print(f"   倉位比例: {self.config.position_pct*100}%")
        print(f"   VIX閾值: {self.config.vix_threshold}")
        print(f"   市場狀態檢查: {self.config.require_market_state}")
...
class TQQQBacktestEngine:
    ...
    def run(
        ...
        for i in range(len(df)):
            ...
            if not self.position:
                ...
                if signal:
                    self._open_position(signal, current_date, current_price)
    ...
    def _open_position(self, signal, timestamp: datetime, current_price: float):
        """開倉"""
        # 考慮滑點
...
def run_backtest():
    ...
    print("="*70)
    print("TQQQ Long/Short Strategy - 2021年度回測")
    print("回測期間: 2021-03-27 至 2022-03-27")
    print("="*70)
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果（主要輸出）
    json_file = f'{report_dir}/backtest_2021_{timestamp}.json'
    result_dict = {
    ...
    # 保存摘要報告
    report_file = f'{report_dir}/backtest_2021_report_{timestamp}.txt'
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("TQQQ Long/Short Strategy - 2021年度回測報告\n")
        f.write("="*70 + "\n\n")
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/backtest_2021_equity_{timestamp}.csv'

...
class TQQQBacktest2025Relaxed:
    ...
    def check_entry_signal(self, df: pd.DataFrame, idx: int) -> tuple:
        ...
        # 做多條件: Z-Score < -1.5 + MACD金叉 + RSI < 35
        ...
        # 做空條件: Z-Score > 1.5 + MACD死叉 + RSI > 65
        if zscore > self.config['entry_zscore']:
...
def main():
    ...
    # 創建回測引擎
    backtest = TQQQBacktest2025Relaxed(**config)
    ...
    output_file = f"{output_dir}/backtest_2025_relaxed.json"

...
class TQQQBacktestStrategy:
    ...
    def check_market_state(self, data: pd.DataFrame, vix_data: pd.DataFrame = None) -> str:
        """檢查市場狀態，包含VIX檢查"""
        if len(data) < 200:
            return 'sideways'
    ...
    def generate_signal(self, data: pd.DataFrame, vix_data: pd.DataFrame = None) -> Optional[Dict[str, Any]]:
        """生成交易信號"""
        ...
        market_state = self.check_market_state(data, vix_data)
...
class TQQQBacktestEngine:
    ...
    def fetch_vix_data(self, period: str = '2y') -> pd.DataFrame:
        """獲取VIX數據"""
        print("📊 正在獲取 VIX 數據...")
        try:
            ...
            print(f"✅ VIX數據獲取成功: {len(df)} 條記錄")
            return df
        except Exception as e:
            print(f"⚠️ VIX數據獲取失敗: {e}")
            return pd.DataFrame()
    ...
    def run(
        ...
        for i in range(len(df)):
            ...
            if not self.position:
                signal = self.strategy.generate_signal(current_data, current_vix)
...
def run_backtest_2024():
    ...
    print("="*70)
    print("TQQQ Long/Short Strategy - 2024年度回測")
    print("="*70)
    ...
    # 獲取數據
    data = engine.fetch_data('TQQQ', period='2y')
    vix_data = engine.fetch_vix_data(period='2y')
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果
    json_file = f'{report_dir}/backtest_2024_{timestamp}.json'
    result_dict = {
        'summary': {
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/equity_curve_2024_{timestamp}.csv'
        result.equity_curve.to_csv(equity_file)
        print(f"📈 權益曲線已保存: {equity_file}")


if __name__ == '__main__':
    result = run_backtest_2024()

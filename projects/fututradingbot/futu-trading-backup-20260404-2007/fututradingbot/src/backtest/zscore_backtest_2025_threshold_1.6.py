"""
Z-Score 策略回測 - 2025年，閾值 1.6
回測期間: 2025-01-01 至 2025-12-31
策略: Z-Score Mean Reversion，閾值 upper=1.6, lower=-1.6
初始資金: $100,000
倉位: 50%
"""
...
class ZScoreBacktestStrategy:
    """
    Z-Score Mean Reversion 策略
    閾值: upper=1.6, lower=-1.6
    """
    
    def __init__(
        self,
        entry_zscore_upper: float = 1.6,     # 做空閾值
        entry_zscore_lower: float = -1.6,    # 做多閾值
    ...
    def check_entry_signal(self, df: pd.DataFrame, idx: int) -> tuple:
        ...
        做多條件: Z-Score < -1.6 + RSI < 35
        做空條件: Z-Score > 1.6 + RSI > 65
        ...
        if zscore < lower_threshold:
            if rsi < self.config['rsi_oversold']:
                if volume > volume_ma * 0.5:
                    return 'long', f"Z-Score({zscore:.2f})<{lower_threshold}+RSI({rsi:.1f})<35"
        
        # 做空條件: Z-Score > 1.6 + RSI > 65
        if zscore > upper_threshold:
            if rsi > self.config['rsi_overbought']:
                if volume > volume_ma * 0.5:
                    return 'short', f"Z-Score({zscore:.2f})>{upper_threshold}+RSI({rsi:.1f})>65"
    ...
    def run_backtest(self, df: pd.DataFrame, start_date: str, end_date: str) -> dict:
        ...
        print(f"Z-Score閾值: upper={self.config['entry_zscore_upper']}, lower={self.config['entry_zscore_lower']}")
...
def main():
    """主函數"""
    print("="*70)
    print("Z-Score 策略回測 - 2025年，閾值 1.6")
    print("="*70)
    ...
    # 策略參數
    config = {
        'entry_zscore_upper': 1.6,      # 做空閾值
        'entry_zscore_lower': -1.6,     # 做多閾值
        'exit_zscore': 0.5,
    ...
    output_file = f"{output_dir}/zscore_backtest_2025_threshold_1.6.json"

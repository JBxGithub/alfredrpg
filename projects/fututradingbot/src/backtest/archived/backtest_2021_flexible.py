"""
FutuTradingBot - 2021年回測（靈活套利策略）
回測期間: 2021-01-01 至 2021-12-31
...
class FlexibleArbitrageStrategy:
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 計算Z-Score (使用較短的回看期以適應2021年數據)
        lookback = min(30, len(df) // 4)  # 動態調整，但至少30天
        lookback = max(lookback, 20)  # 最少20天
        ...
        # 計算100日均線 (如果數據不足，使用可用數據的最大長度)
        available_days = len(df)
        ma_period = min(self.ma_period, available_days - 1)
        ma_period = max(ma_period, 30)  # 至少30天
        df['ma100'] = df['close'].rolling(window=ma_period).mean()
        
        return df
    
    def determine_market_state(self, price: float, ma100: float, vix: Optional[float] = None) -> MarketState:
        """
        判斷市場狀態
        ...
        if vix is None:
            # 簡化判斷：基於價格與100日均線的關係
            deviation = (price - ma100) / ma100
            
            if deviation > 0.05:  # 價格高於100日均線5%以上
                return MarketState(
                    state="bull",
            ...
            elif deviation < -0.05:  # 價格低於100日均線5%以上
                return MarketState(
                    state="bear",
        ...
        # 有VIX數據時的判斷
        # 牛市條件: 價格在100日均線之上 + VIX低
        if price > ma100 and vix < self.vix_bull_threshold:
            return MarketState(
                state="bull",
                primary_direction="long",
                zscore_threshold=self.zscore_threshold_bull,
        ...
        # 熊市條件: 價格在100日均線之下 + VIX高
        elif price < ma100 and vix > self.vix_bear_threshold:
            return MarketState(
                state="bear",
                primary_direction="short",
                zscore_threshold=self.zscore_threshold_bear,
    ...
    def generate_signal(self, data: pd.DataFrame, vix_data: Optional[float] = None) -> Optional[Dict[str, Any]]:
        ...
        # 檢查數據有效性
        if pd.isna(current['zscore']) or pd.isna(current['ma100']):
            return None
...
def run_backtest():
    ...
    print("="*70)
    print("TQQQ 靈活套利策略 - 2021年度回測")
    print("="*70)
    print("回測期間: 2021-01-01 至 2021-12-31")
...
def save_results(result: BacktestResult, strategy: FlexibleArbitrageStrategy):
    ...
    # 保存JSON結果
    json_file = f'{report_dir}/backtest_2021_flexible.json'
    result_dict = {

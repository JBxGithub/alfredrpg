...
from src.strategies.tqqq_long_short import TQQQStrategyConfig
...
class TQQQBacktestStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 存儲額外參數（不在TQQQStrategyConfig中）
        self.vix_threshold = 25.0
        
        if config:
            if 'vix_threshold' in config:
                self.vix_threshold = config.pop('vix_threshold')
            default_config.update(config)
        
        self.config = TQQQStrategyConfig(**default_config)
        self.current_positions = []
        
        print(f"TQQQ策略初始化: 倉位{self.config.position_pct*100}%, Z-Score±{self.config.entry_zscore}, VIX閾值{self.vix_threshold}")
    ...
    def check_vix_condition(self, data: pd.DataFrame) -> bool:
        ...
        # 如果波動率過高，暫停交易
        return volatility_pct < self.vix_threshold
    ...
    def generate_signal(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        # VIX檢查（如果啟用）
        if self.config.require_market_state:
            if not self.check_vix_condition(data):
                return None
...
def run_backtest():
    ...
    print("="*70)
    print("TQQQ Long/Short Strategy - 2022年度回測")
    print("="*70)
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果（符合要求的命名格式）
    json_file = f'{report_dir}/backtest_2022_{timestamp}.json'
    result_dict = {
    ...
    # 保存摘要報告
    report_file = f'{report_dir}/backtest_2022_report_{timestamp}.txt'
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("TQQQ Long/Short Strategy - 2022年度回測報告\n")
        f.write("="*70 + "\n\n")
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/equity_curve_2022_{timestamp}.csv'

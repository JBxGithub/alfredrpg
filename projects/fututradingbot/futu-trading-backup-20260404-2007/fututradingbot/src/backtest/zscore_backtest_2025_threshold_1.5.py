"""
Z-Score 策略回測 - 2025年，閾值 1.5
回測期間: 2025-01-01 至 2025-12-31 (全年)
策略: Z-Score Mean Reversion，閾值 upper=1.5, lower=-1.5
...
class ZScoreBacktestThreshold15:
    """
    Z-Score Mean Reversion 策略回測
    閾值: upper=1.5, lower=-1.5
    """
    
    def __init__(
        self,
        upper_threshold: float = 1.5,      # 上閾值
        lower_threshold: float = -1.5,     # 下閾值
        ...
        zscore_lookback: int = 60,         # Z-Score計算週期
        initial_capital: float = 100000.0,
        commission_rate: float = 0.001,    # 手續費 0.1%
    ...
    def check_exit_signal(self, df: pd.DataFrame, idx: int, position: dict) -> tuple:
        ...
        if direction == 'long' and zscore > -exit_thresh:
            return True, f"Z-Score回歸({zscore:.2f})"
        if direction == 'short' and zscore < exit_thresh:
            return True, f"Z-Score回歸({zscore:.2f})"
    ...
    def run_backtest(self, df: pd.DataFrame, start_date: str, end_date: str) -> dict:
        ...
        print(f"Z-Score閾值: upper={self.config['upper_threshold']}, lower={self.config['lower_threshold']}")
        ...
        for i in range(len(df)):
            ...
            self.equity_curve.append({
                ...
                'close': current_price,
                'zscore': df['zscore'].iloc[i] if not pd.isna(df['zscore'].iloc[i]) else None
            })
    ...
    def generate_report(self, df: pd.DataFrame) -> dict:
        ...
        report = {
            ...
            'strategy_name': 'Z-Score Mean Reversion (Threshold 1.5)',
...
def main():
    """主函數"""
    print("="*70)
    print("Z-Score 策略回測 - 2025年，閾值 1.5")
    print("="*70)
    ...
    # 創建回測引擎
    backtest = ZScoreBacktestThreshold15(**config)
    ...
    print(f"Z-Score閾值: upper={report['strategy_config']['upper_threshold']}, lower={report['strategy_config']['lower_threshold']}")
    ...
    print(f"最大回撤: ${report['max_drawdown']:,.2f} ({report['max_drawdown_pct']:.2f}%)")
    ...
    output_file = f"{output_dir}/zscore_backtest_2025_threshold_1.5.json"

...
from src.analysis.mtf_analyzer import MTFAnalyzer, TrendDirection
...
class TQQQStrategyConfig:
    ...
    # MTF整合配置
    require_market_state: bool = True   # 是否檢查市場狀態
    use_mtf: bool = True                # 是否使用MTF分析
    mtf_min_score: float = 60.0         # MTF最低評分要求
    use_macdv: bool = True              # 是否使用MACD-V
    use_divergence: bool = True         # 是否使用背離檢測
...
class TQQQLongShortStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        default_config = {
            ...
            'time_stop_days': 7,      # 回測優化（統一為7天）
            'take_profit_pct': 0.05,
            'stop_loss_pct': 0.03,
            'require_market_state': True,
            # MTF整合配置
            'use_mtf': True,
            'mtf_min_score': 60,      # MTF最低評分要求
            'use_macdv': True,        # 啟用MACD-V
            'use_divergence': True    # 啟用背離檢測
        }
        ...
        # 初始化MTF分析器（新增）
        if default_config.get('use_mtf', True):
            self.mtf_analyzer = MTFAnalyzer()
            logger.info("TQQQ策略: MTF分析已啟用")
        else:

"""
TQQQ 5年回測腳本 - TQQQ 5-Year Backtest Script
執行TQQQ單邊多空策略的5年回測（2021-2026）
"""
...
class TQQQBacktestStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        print(f"TQQQ策略初始化: 倉位{self.config.position_pct*100}%, Z-Score±{self.config.entry_zscore}")
...
class TQQQBacktestEngine:
    ...
    def fetch_data(self, symbol: str = 'TQQQ', period: str = '5y') -> pd.DataFrame:
        ...
        # 處理索引（Yahoo Finance返回的索引通常是Date）
        df = df.reset_index()
    ...
    def _open_position(self, signal, timestamp: datetime, data: pd.DataFrame):
        ...
        if isinstance(signal_type, type(signal_type)) and hasattr(signal_type, 'value'):
...
def run_backtest():
    ...
    print("="*70)
    print("TQQQ Long/Short Strategy - 5年回測")
    print("="*70)
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存摘要報告
    report_file = f'{report_dir}/backtest_report_{timestamp}.txt'
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("TQQQ Long/Short Strategy - 5年回測報告\n")
        f.write("="*70 + "\n\n")
    ...
    # 保存JSON結果
    json_file = f'{report_dir}/backtest_result_{timestamp}.json'
    result_dict = {

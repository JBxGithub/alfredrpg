"""
TQQQ 2024年度回測腳本（原始策略參數）
回測期間: 2024-01-01 至 2024-12-31
策略參數: 原始參數（Entry Z-Score 2.0, Exit Z-Score 0.5, Position 50%, Time Stop 3天）
...
class TQQQBacktestStrategyOriginal:
    """TQQQ回測專用策略（原始參數版）"""
...
class TQQQBacktestEngine:
    ...
    def __init__(
        self,
        strategy: TQQQBacktestStrategyOriginal,
...
def run_backtest():
    ...
    print("="*70)
    print("TQQQ Long/Short Strategy - 2024年度回測（原始參數版）")
    print("="*70)
    ...
    # 創建策略
    strategy = TQQQBacktestStrategyOriginal(config)
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategyOriginal):
    """保存回測結果"""
    ...
    # 保存JSON結果（符合要求的命名格式）
    json_file = f'{report_dir}/backtest_2024_original.json'
    result_dict = {

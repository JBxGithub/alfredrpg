...
class TQQQStrategyConfig:
    ...
    rsi_period: int = 14
    rsi_overbought: float = 70.0  # 最終版
    rsi_oversold: float = 30.0    # 最終版
...
class TQQQBacktestStrategy:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        default_config = {
            ...
            'lookback_period': 60,
            'rsi_period': 14,
            'rsi_overbought': 70.0,  # 最終版
            'rsi_oversold': 30.0,    # 最終版
            'volume_ma_period': 20,
        ...
        print(f"TQQQ策略初始化(最終版): 倉位{self.config.position_pct*100}%, Z-Score±{self.config.entry_zscore}")
        print(f"  RSI閾值: {self.config.rsi_oversold}/{self.config.rsi_overbought}")
        print(f"  進場條件: Z-Score + MACD金叉/死叉 + RSI")
    ...
    def generate_signal(self, data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        進場條件:
        - 做多: Z-Score < -2.0 + (MACD金叉 或 RSI < 30) + 市場狀態允許
        - 做空: Z-Score > 2.0 + (MACD死叉 或 RSI > 70) + 市場狀態允許
        
        註：使用「或」條件而非「與」條件，確保在2023年這樣的牛市中仍有合理交易機會
        """
        ...
        # MACD交叉檢測
        macd_golden_cross = prev_macd_hist < 0 and macd_hist >= 0  # MACD金叉
        macd_death_cross = prev_macd_hist > 0 and macd_hist <= 0   # MACD死叉
        
        # ========== 做多條件檢查（最終版 - 優化）==========
        # 條件：Z-Score < -2.0 + (MACD金叉 或 RSI < 30) + 成交量確認 + 市場狀態允許
        if zscore < -self.config.entry_zscore:
            rsi_condition = rsi < self.config.rsi_oversold  # RSI < 30
            volume_condition = volume > volume_ma * 0.8
            market_condition = market_state in ['bull', 'sideways']
            
            # 使用「或」條件：MACD金叉 或 RSI超賣
            confirmation = macd_golden_cross or rsi_condition
            
            if confirmation and volume_condition and market_condition:
                reason = f"Z-Score({zscore:.2f})+"
                if macd_golden_cross and rsi_condition:
                    reason += f"RSI({rsi:.1f})+MACD金叉+{market_state}市"
                elif macd_golden_cross:
                    reason += f"MACD金叉+{market_state}市"
                else:
                    reason += f"RSI({rsi:.1f})+{market_state}市"
        ...
        # ========== 做空條件檢查（最終版 - 優化）==========
        # 條件：Z-Score > 2.0 + (MACD死叉 或 RSI > 70) + 成交量確認 + 市場狀態允許
        if zscore > self.config.entry_zscore:
            rsi_condition = rsi > self.config.rsi_overbought  # RSI > 70
            volume_condition = volume > volume_ma * 0.8
            market_condition = market_state in ['bear', 'sideways']
            
            # 使用「或」條件：MACD死叉 或 RSI超買
            confirmation = macd_death_cross or rsi_condition
            
            if confirmation and volume_condition and market_condition:
                reason = f"Z-Score({zscore:.2f})+"
                if macd_death_cross and rsi_condition:
                    reason += f"RSI({rsi:.1f})+MACD死叉+{market_state}市"
                elif macd_death_cross:
                    reason += f"MACD死叉+{market_state}市"
                else:
                    reason += f"RSI({rsi:.1f})+{market_state}市"
...
class TQQQBacktestEngine:
    ...
    def fetch_data(self, symbol: str = 'TQQQ', start_date: str = None, end_date: str = None) -> pd.DataFrame:
        """從Yahoo Finance獲取數據"""
    ...
    def run(
        ...
        for i in range(len(df)):
            ...
            if self.position:
                ...
                self.position['current_value'] = self.position['shares'] * current_price
    ...
    def _open_position(self, signal, timestamp: datetime, data: pd.DataFrame):
        ...
        if hasattr(signal_type, 'value'):
            ...
            signal_value = str(signal_type).lower()
    ...
    def _record_equity(self, timestamp: datetime, current_price: float):
        ...
        position_value = 0
        if self.position:
            position_value = self.position.get('current_value', 0)
...
def run_backtest_2023_final():
    """執行2023年度回測（最終版）"""
    
    print("="*70)
    print("TQQQ Long/Short Strategy - 2023年度回測（最終版）")
    print("="*70)
    print("回測期間: 2023-01-01 至 2023-12-31")
    print("策略版本: 框架A最終版（含MACD進場條件）")
    print("="*70)
    ...
    # 獲取數據（獲取更長的數據以計算指標）
    data = engine.fetch_data('TQQQ', start_date='2022-01-01', end_date='2024-06-30')
    ...
    # 輸出結果
    print("\n" + "="*70)
    print("📊 2023年度回測結果（最終版）")
    print("="*70)
    print(f"回測期間: {result.start_date.strftime('%Y-%m-%d')} ~ {result.end_date.strftime('%Y-%m-%d')}")
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果 - 固定檔名
    json_file = f'{report_dir}/backtest_2023_final.json'
    result_dict = {
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/equity_curve_2023_final.csv'
        result.equity_curve.to_csv(equity_file)
        print(f"📈 權益曲線已保存: {equity_file}")


if __name__ == '__main__':
    result = run_backtest_2023_final()

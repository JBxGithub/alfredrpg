...
class SixLoopBacktestV9_3:
    def __init__(self, start_date='2019-01-01', end_date='2024-12-31', initial_capital=100000):
        ...
        # 分層止損
        self.stop_loss_levels = [0.02, 0.03, 0.05]  # -2%, -3%, -5%
        self.stop_loss_sizes = [0.5, 0.3, 1.0]      # 減50%, 再減30%, 全平
        
        # 盈利保護
        self.profit_levels = [0.10, 0.15, 0.20]     # +10%, +15%, +20%
        self.profit_sizes = [0.0, 0.5, 0.3]         # 移損, 止盈50%, 再止盈30%
        self.profit_trailing = 0.03
        
        # 空單
        self.short_stop = 0.02
        self.short_take = 0.10
    ...
    def run_backtest(self):
        print("\n" + "="*70)
        print("六循環系統回測 - V9.3 完整版")
        print("="*70)
        print("【分層止損】-2%減50% → -3%再減30% → -5%全平")
        print("【盈利保護】+10%移損 → +15%止盈50% → +20%再止盈30%")
        print("【VIX過濾】>30減倉50% → >35全平")
        print("="*70)
        ...
        for i in range(len(common_dates)):
            ...
            if self.position != 0:
                ...
                if self.position > 0:  # 多單
                    ...
                    # 分層止損
                    if qqq_change <= -self.stop_loss_levels[2]:  # -5%
                        signal = 'SELL_LONG_STOP_FULL'
                        reason = f'全平: -{abs(qqq_change)*100:.1f}%'
                    elif qqq_change <= -self.stop_loss_levels[1] and self.current_shares > self.initial_shares * 0.2:  # -3%
                        sell_shares = int(self.initial_shares * 0.3)
                        if sell_shares > 0 and self.current_shares >= sell_shares:
                            signal = 'SELL_LONG_PARTIAL_2'
                            reason = f'再減30%: -{abs(qqq_change)*100:.1f}%'
                            self.current_shares -= sell_shares
                            self.shares_sold += sell_shares
                    elif qqq_change <= -self.stop_loss_levels[0] and self.current_shares > self.initial_shares * 0.5:  # -2%
                        sell_shares = int(self.initial_shares * 0.5)
                        if sell_shares > 0 and self.current_shares >= sell_shares:
                    ...
                    # 盈利保護
                    elif etf_change >= self.profit_levels[2] and self.current_shares > self.initial_shares * 0.2:  # +20%
                        sell_shares = int(self.initial_shares * 0.3)
                        if sell_shares > 0 and self.current_shares >= sell_shares:
                            signal = 'SELL_LONG_PROFIT_2'
                            reason = f'再止盈30%: +{etf_change*100:.1f}%'
                            self.current_shares -= sell_shares
                            self.shares_sold += sell_shares
                    elif etf_change >= self.profit_levels[1] and self.current_shares > self.initial_shares * 0.5:  # +15%
                        sell_shares = int(self.initial_shares * 0.5)
                        if sell_shares > 0 and self.current_shares >= sell_shares:
                    ...
                    elif etf_change >= self.profit_levels[0]:  # +10%
                        self.breakeven_activated = True
                        signal = 'HOLD_LONG_BREAKEVEN'
                        reason = f'移動止損激活: +{etf_change*100:.1f}%'
                    
                    # 移動止損
                    elif self.breakeven_activated and etf_price <= self.entry_price * 1.0:
                        signal = 'SELL_LONG_BREAKEVEN_STOP'
                        reason = '回落成本價，全平'
                    ...
                    elif vix_price > self.vix_reduce and self.current_shares > self.initial_shares * 0.5:
                        sell_shares = int(self.initial_shares * 0.5)
                        if sell_shares > 0 and self.current_shares >= sell_shares:
                    ...
                    elif etf_change >= self.short_take:
            ...
            if 'BUY' in signal and self.position == 0:
                ...
                if shares > 0:
                    ...
                    self.holding_days = 0
                    self.initial_shares = abs(self.position)
                    self.current_shares = abs(self.position)
                    self.shares_sold = 0
                    self.breakeven_activated = False
                ...
                if 'PARTIAL' in signal or 'PROFIT' in signal or 'VIX_REDUCE' in signal:
                    # 部分平倉
                    sell_shares = self.initial_shares - self.current_shares - self.shares_sold
                    if sell_shares > 0:
                        if self.position > 0:
                            gross_pnl = (etf_price - self.entry_price) * sell_shares
                        else:
                            gross_pnl = (self.entry_price - etf_price) * sell_shares
                        ...
                        # 更新持倉
                        self.position = self.current_shares if self.position > 0 else -self.current_shares
    ...
    def calculate_metrics(self):
        ...
        partial_trades = [t for t in sell_trades if 'PARTIAL' in t['action'] or 'PROFIT' in t['action']]
        full_trades = [t for t in sell_trades if 'FULL' in t['action'] or 'STOP' in t['action'] or 'VIX_EXIT' in t['action']]
        ...
        print(f"  部分平倉: {len(partial_trades)} 次")
        print(f"  全部平倉: {len(full_trades)} 次")
        print(f"勝率: {win_rate:.1f}%")
        
        buy_hold_tqqq = (self.tqqq_data['Close'].iloc[-1] / self.tqqq_data['Close'].iloc[0] - 1) * 100
        
        print(f"\n【策略對比】")
        print(f"V9.3 策略: {total_return:.2f}%")

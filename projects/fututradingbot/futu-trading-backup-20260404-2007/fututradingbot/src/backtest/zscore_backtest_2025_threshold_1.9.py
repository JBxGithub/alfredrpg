"""
TQQQ Z-Score 策略回測 - 2025年，閾值 1.9
回測期間: 2025-01-01 至 2025-12-31 (全年)
策略: Z-Score Mean Reversion，固定閾值 ±1.9

## 策略參數
- Z-Score 閾值: upper=1.9, lower=-1.9
- 初始資金: $100,000
- 倉位: 50%
- 止盈: 5%
- 止損: 3%
...
class ZScoreStrategy:
    ...
    def __init__(
        self,
        zscore_upper: float = 1.9,      # Z-Score 上閾值 (做空)
        zscore_lower: float = -1.9,     # Z-Score 下閾值 (做多)
        exit_zscore: float = 0.5,       # Z-Score 回歸出場
    ...
    def check_entry_signal(self, df: pd.DataFrame, idx: int) -> tuple:
        ...
        做多條件: Z-Score < -1.9 + RSI確認
        做空條件: Z-Score > 1.9 + RSI確認
        ...
        # 做多條件: Z-Score < -1.9 + (RSI超賣 或 MACD金叉)
        ...
        # 做空條件: Z-Score > 1.9 + (RSI超買 或 MACD死叉)
    ...
    def run_backtest(self, df: pd.DataFrame, start_date: str, end_date: str) -> dict:
        ...
        print(f"Z-Score閾值: upper={self.config['zscore_upper']}, lower={self.config['zscore_lower']}")
    ...
    def generate_report(self, df: pd.DataFrame) -> dict:
        ...
        report = {
            ...
            'strategy_name': 'Z-Score Mean Reversion (Threshold 1.9)',
...
def main():
    """主函數"""
    print("="*70)
    print("TQQQ Z-Score 策略回測 - 2025年 (閾值 1.9)")
    print("="*70)
    ...
    # 策略參數
    config = {
        'zscore_upper': 1.9,       # Z-Score 上閾值
        'zscore_lower': -1.9,      # Z-Score 下閾值
        'exit_zscore': 0.5,
    ...
    print(f"Z-Score閾值: upper={report['strategy_config']['zscore_upper']}, lower={report['strategy_config']['zscore_lower']}")
    ...
    output_file = f"{output_dir}/zscore_backtest_2025_threshold_1.9.json"

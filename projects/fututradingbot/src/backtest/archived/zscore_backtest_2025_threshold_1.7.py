"""
TQQQ Z-Score策略回測 - 閾值 1.7
回測期間: 2025年全年
策略: Z-Score Mean Reversion，固定閾值 ±1.7

## 策略邏輯
- Z-Score > 1.7: 做空（價格偏離過高，預期回歸）
- Z-Score < -1.7: 做多（價格偏離過低，預期回歸）
- Z-Score回歸到 ±0.5: 平倉
...
class ZScoreStrategy:
    """
    Z-Score Mean Reversion 策略 - 固定閾值版本
    """
    
    def __init__(
        self,
        zscore_upper: float = 1.7,      # 做空閾值
        zscore_lower: float = -1.7,     # 做多閾值
        exit_zscore: float = 0.5,       # 出場閾值
        position_pct: float = 0.50,     # 50%倉位
        take_profit_pct: float = 0.05,  # 5%止盈
        stop_loss_pct: float = 0.03,    # 3%止損
        time_stop_days: int = 5,        # 5天時間止損
        ...
        self.config = {
            'zscore_upper': zscore_upper,
            'zscore_lower': zscore_lower,
            ...
            'zscore_lookback': 60,       # Z-Score計算週期
            'rsi_period': 14,
            'volume_ma_period': 20,
            'stop_loss_zscore': 3.5      # Z-Score極端止損
        }
    ...
    def check_entry_signal(self, df: pd.DataFrame, idx: int) -> tuple:
        ...
        upper_threshold = self.config['zscore_upper']
        lower_threshold = self.config['zscore_lower']
        
        # 做多條件: Z-Score < -1.7 + (RSI超賣 或 MACD金叉)
        if zscore < lower_threshold:
            ...
            if rsi_confirm or macd_confirm:
                if volume > volume_ma * 0.5:
                    ...
                    return 'long', f"Z-Score({zscore:.2f})<{lower_threshold}+{confirm_str}確認"
        
        # 做空條件: Z-Score > 1.7 + (RSI超買 或 MACD死叉)
        if zscore > upper_threshold:
            ...
            if rsi_confirm or macd_confirm:
                if volume > volume_ma * 0.5:
                    confirm_str = "RSI" if rsi_confirm else "MACD"
                    return 'short', f"Z-Score({zscore:.2f})>{upper_threshold}+{confirm_str}確認"
    ...
    def run_backtest(self, df: pd.DataFrame, start_date: str, end_date: str) -> dict:
        ...
        print(f"Z-Score閾值: 做多={self.config['zscore_lower']}, 做空={self.config['zscore_upper']}")
        ...
        for i in range(len(df)):
            ...
            if not self.position:
                ...
                if signal_type:
                    ...
                    position_value = net_equity * self.config['position_pct']
    ...
    def generate_report(self, df: pd.DataFrame) -> dict:
        ...
        report = {
            ...
            'strategy_name': 'Z-Score Mean Reversion (Threshold 1.7)',
...
def main():
    ...
    # 策略參數
    config = {
        'zscore_upper': 1.7,       # 做空閾值
        'zscore_lower': -1.7,      # 做多閾值
        'exit_zscore': 0.5,
    ...
    # 創建回測引擎
    backtest = ZScoreStrategy(**config)
    ...
    print(f"Z-Score閾值: {report['strategy_config']['zscore_lower']} / {report['strategy_config']['zscore_upper']}")
    ...
    print(f"平均持有天數: {report['avg_hold_days']:.2f}天")
    print("="*70)
    ...
    output_file = f"{output_dir}/zscore_backtest_2025_threshold_1.7.json"

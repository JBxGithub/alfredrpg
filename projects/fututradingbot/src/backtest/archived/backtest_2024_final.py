...
class TQQQStrategyConfig:
    ...
    rsi_period: int = 14
    rsi_overbought: float = 70.0  # 最終版: 70
    rsi_oversold: float = 30.0    # 最終版: 30
...
def run_backtest_2024_final():
    """執行2024年度回測（最終版）"""
    
    print("="*70)
    print("TQQQ Long/Short Strategy - 2024年度回測（最終版）")
    print("="*70)
...
def save_results(result: BacktestResult, strategy: TQQQBacktestStrategy):
    ...
    # 保存JSON結果 - 使用指定文件名
    json_file = f'{report_dir}/backtest_2024_final.json'
    ...
    if not result.equity_curve.empty:
        equity_file = f'{report_dir}/equity_curve_2024_final.csv'
        result.equity_curve.to_csv(equity_file)
        print(f"📈 權益曲線已保存: {equity_file}")


if __name__ == '__main__':
    result = run_backtest_2024_final()

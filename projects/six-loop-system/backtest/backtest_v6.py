...
class SixLoopBacktestV6:
    ...
    def __init__(self, start_date='2019-01-01', end_date='2024-12-31', initial_capital=100000):
        ...
        self.holding_days = 0
        self.cooldown_days = 0  # 冷卻期計數器
        self.trades = []
        
        # 核心參數
        self.qqq_stop_loss = 0.02
        self.qqq_trailing_stop = 0.02
        ...
        for i in range(len(common_dates)):
            ...
            if self.positions > 0:
                ...
                if qqq_change_from_entry <= -self.qqq_stop_loss:
                    ...
                    self.cooldown_days = self.cooldown_period  # 啟動冷卻期
    ...
    def calculate_metrics(self):
        ...
        cooldown_days = len([s for s in self.daily_results['signal'] if 'COOLDOWN' in s])
    ...
    def save_results(self):
        ...
        self.daily_results.to_csv(f'backtest/results/v6_daily_{timestamp}.csv', index=False)
        
        if self.trades:
            trades_df = pd.DataFrame(self.trades)
            trades_df.to_csv(f'backtest/results/v6_trades_{timestamp}.csv', index=False)
        
        with open(f'backtest/results/v6_metrics_{timestamp}.json', 'w') as f:
            json.dump(self.metrics, f, indent=2, default=str)
        
        print(f"\n結果已保存: backtest/results/v6_*_{timestamp}.*")

def main():
    backtest = SixLoopBacktestV6(
